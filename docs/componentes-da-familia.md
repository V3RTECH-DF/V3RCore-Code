...
| **Navegação do painel** | entrada única no menu, navegação interna, permissão e bloqueio de endereço, posição e contiguidade da família na coluna do painel | 0.22.0 | **os 9** — Flow, V3RLGPD e V3RHelp (1.33.0) pela camada inteira; GE Associados só declaração e guardas; quatro pela porta barata; V3RLicense por escrita direta, sem embutir a biblioteca | `navegacao-do-painel.md` |
| **Endereços antigos** | o endereço salvo de um submenu que deixou de existir continua funcionando — inclusive preservando parâmetros quando o destino é rota interna ou outra página do painel; e inclusive quando o slug antigo não coincide com nenhuma página registrada, caso em que o WordPress recusava com 403 antes de `admin_init` disparar | 0.19.0 (slug de página do painel: 0.23.0; slug não registrado por ninguém: 0.26.0, `#40`) | V3RLGPD | `navegacao-do-painel.md` §7 |
