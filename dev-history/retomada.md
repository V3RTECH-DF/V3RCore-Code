...
| RIT360 Premiado | 2.32.0 ✅ | fatia 4 corrigida com v0.7.2, reinstalada | Ordem do aviso corrigida; "fonte fora da Exo 2" não reproduziu (provável instrumento sem escopo). Configurações em 375 com 1026px (antes 942): **aceito (13/09)** — é o `.text-sm` do produto que o wp-admin anulava + Exo 2; a #214 (sub-abas quebrando linha) sobe de prioridade. Remedição em curso (`scratchpad/premiado-final/` desta sessão). Página pública idêntica ✅. |
...
| RIT360 Solidário | 2.28.0 ✅ | fatia 4 instalada no dev-wp (branch feat/76-fatia4-visual, sem bump); medição do "depois" em curso (`scratchpad/solidario-depois/` desta sessão) | Foto do "antes" com a sessão anterior (`solidario-antes/`). Build decidido: **um `vite.config` só** (a página pública sai com sha256 idêntico; dois configs mudavam os chunks públicos), com guarda de build que falha se a entrada pública passar a importar o pacote ou o `index.css`. Plano 1–5 da RIT360-Solidario-Code#76 aprovado. Cor `--v3r-accent: #24ae9c`. Não instalar no dev-wp antes de a foto terminar. |
...
- V3RCore-Code#49 (restauração de checkbox/radio/aviso do wp-admin aplicada também fora do wp-admin — azul do wp-admin na gestão pública do V3RLGPD); correção v0.7.3 com agente no `Front/` desde 13/09 — **não rodar `sync -f` até ele terminar**. Não bloqueia o V3RLGPD (sobrepõe a marca no produto).
