...
| RIT360 Solidário | 2.28.0 ✅ | fatia 4 medida (13/09, `scratchpad/solidario-depois/` desta sessão): 9/9 critérios, rolagem em 375 zerou. ✅ **2.29.0 publicada** (14/09; cadeia pública com sha256 idêntico no ZIP da release). Concluído; a sessão fecha a #76 depois do manual | Foto do "antes" com a sessão anterior (`solidario-antes/`). Build decidido: **um `vite.config` só** (a página pública sai com sha256 idêntico; dois configs mudavam os chunks públicos), com guarda de build que falha se a entrada pública passar a importar o pacote ou o `index.css`. Plano 1–5 da RIT360-Solidario-Code#76 aprovado. Cor `--v3r-accent: #24ae9c`. Não instalar no dev-wp antes de a foto terminar. |
