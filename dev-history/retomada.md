...
- `@v3rtech/v3r-front` **v0.7.3** ✅ publicada em 14/09 (tag `v0.7.3` → acc9dc0): restauração de checkbox/radio/aviso do wp-admin só dentro do wp-admin (#49).
...
| V3REvent | 1.87.0 ✅ | fatia 4 final instalada (Check-in por entrada fina e80e02c/d9aac08, rótulos 59c4440, v0.7.2 0abc0e0; v3r-core 0.25.0 mantida — sem endereço antigo); remedição (14/09, `scratchpad/v3revent-final/` desta sessão) passou nos 9 critérios; checkbox nativo no site aceito; A/B de "Shortcodes e API" em 375: 1.87.0 1106px → fatia 4 1020px (melhorou; transbordo pré-existente vira issue do produto). ✅ **1.88.0 publicada** (14/09) no site da V3RTECH (assets com sha256 do ZIP) e pela atualização automática (checksum do servidor de licenças = ZIP do GitHub); mu-plugin apagado; dev-wp com o ZIP da release. Concluído. Publicação: site da V3RTECH + atualização automática; a sessão confirma com o Bruno quem executa | Check-in em branco: pedaços sob demanda importavam `./admin.js`/`./front.js`, e a query `?ver=` gerava uma segunda cópia do React. A correção (entrada fina com `import()` do app) está com o implementador. Rótulos da barra: "API" volta a "Shortcodes e API"; "Painel"/"Dashboard" seguem a opção (c), cada superfície com o rótulo de hoje, e há issue para o Bruno unificar. Depois: subir para v0.7.2, remedir (viewport criada já em 375, `matchMedia` registrado, padding do aviso) e liberar. |
| RIT360 Premiado | 2.32.0 ✅ | fatia 4 corrigida com v0.7.2, reinstalada | Ordem do aviso corrigida; "fonte fora da Exo 2" não reproduziu (provável instrumento sem escopo). Configurações em 375 com 1026px (antes 942): **aceito (13/09)** — é o `.text-sm` do produto que o wp-admin anulava + Exo 2; a #214 (sub-abas quebrando linha) sobe de prioridade. Remedição (13/09, `scratchpad/premiado-final/` desta sessão): tudo passou, exceto duas telas que rolam em 375 sem medida do "antes" (Nova campanha 386, Auditoria 497) — A/B com o dist da 2.32.0 (14/09): Auditoria 495→497 (Exo 2, aceito); Nova campanha 375→386, rolagem nova por item de grid sem `min-width:0` e stepper sem quebra (fragilidade do produto) → corrigido no produto (min-w-0; causa real: barra do TinyMCE; rótulos do stepper mantidos ocultos no celular), remedido 375 sem rolagem e 1440 idêntico (`scratchpad/premiado-novacampanha/`). ✅ **2.33.0 publicada** (14/09) pela atualização automática (checksum do servidor de licenças = ZIP) e no site da V3RTECH (que estava em 2.31.0 — o deploy da publicação 1 não tinha chegado lá; agora 2.33.0, HTTP 200); mu-plugin apagado. Concluído; a sessão fecha a #212 depois de manual e docs. Publicação: site da V3RTECH + atualização automática (orientação do Bruno, 14/09). `@layer` na folha pública é o mesmo hash da 2.32.0 (não conta). Página pública idêntica ✅. |
...
| RIT360 Solidário | 2.28.0 ✅ | ✅ **2.29.0 publicada** (14/09; cadeia pública com sha256 idêntico no ZIP da release) | Concluído; a sessão fecha a #76 depois do manual. Medição do "depois" (13/09, `scratchpad/solidario-depois/` desta sessão): 9/9 critérios, rolagem em 375 zerou. Build decidido: **um `vite.config` só** (a página pública sai com sha256 idêntico; dois configs mudavam os chunks públicos), com guarda de build que falha se a entrada pública passar a importar o pacote ou o `index.css`. Plano 1–5 da RIT360-Solidario-Code#76 aprovado. Cor `--v3r-accent: #24ae9c`. |
| V3RLicense | — (só visual) | 0.35.0 com v0.7.2 instalada no dev-wp (211 arquivos conferidos contra o ZIP) | Sessão v3rlicense-d3. Remedição (13/09, `scratchpad/v3rlicense-depois/` desta sessão) passou nos 8 critérios. ✅ **0.35.0 publicada** (13/09; V3RLicense-Code#64 fechada, dados de teste e mu-plugin apagados). Concluído. |
| V3RLGPD | já migrado (1.79.0) | — | Sessão v3rlgpd-ca republicando com v3r-core 0.26.0, v3r-front #v0.7.2 e avisos abaixo da barra (V3RLGPD-Code#118/#119). "Antes" de acesso medido em 13/09 (sonda `bin/sondas/v3rlgpd.env`, saída em `scratchpad/v3rlgpd-antes/acesso/` desta sessão): administrador redireciona em ripd/policies/settings e recebe 403 em auditoria/ropa/atendimento/docs (4, não 5). "Depois": os 7 redirecionam para quem está logado (auditoria/ropa/atendimento para as rotas de índice, docs para #/manual — aceito), resto da matriz idêntico. Fica na v3r-front 0.7.2 com a marca do checkbox sobreposta (a 0.7.3 devolveria a caixa 2×2 na gestão no site; V3RLGPD-Code#120). ZIP instalado 14/09 00:04 (CI verde em 127c1c5; build limpo com os mesmos hashes). Acesso do "depois" medido: administrador redireciona nos 7; operador/assinante vão à expulsão do WooCommerce em auditoria/ropa/atendimento/docs e seguem com 403 em ripd/policies/settings, igual ao antes — defeito da biblioteca registrado em V3RCore-Code#50, não bloqueia. Visual do "depois" (14/09, `scratchpad/v3rlgpd-depois/visual/`): passou (título visível, rolagem zerou, marca do checkbox preservada, 302 nos 7; gestão no site sem página no dev-wp — vale a medição Chromium da sessão). ✅ **1.79.1 publicada** (14/09) pela atualização automática (ZIP da release com CSS/JS idênticos à cópia medida) e no site da V3RTECH (HTTP 200, assets ver=1.79.1); mu-plugin apagado; V3RLGPD-Code#118/#119 fechadas. Depois saiu uma **1.79.2** (14/09 00:24, disparada pelo Bruno; só pontos de versão, ZIP igual à cópia medida fora os metadados de versão), já no site da V3RTECH (assets ver=1.79.2). Concluído. Visual do "antes" feito (`scratchpad/v3rlgpd-antes/visual/`): título some em 375 (v0.6.0), rolagem 400/375, checkbox/radio 16→25 visíveis, Exo 2 total, console limpo, sem aviso para comparar posição. |
...
- V3RCore-Code#50 (LegacyRedirects: endereço antigo que coincide com tela real dá 403 para quem não tem a permissão da tela; avaliar `admin_page_access_denied`)
